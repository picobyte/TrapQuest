...
Definition: yourself is glue stuck:
	...
	if there is glue grabbing the player, decide yes;
	decide no.
...
Definition: yourself is glued seductively:
	...
	repeat with X running through all glue:
		if X is grabbing the thing and the smell-duration of X > 0:
			[if debugmode is 1, say "DEBUG: Thing is glue stuck and attractive.";]
			decide yes;
	decide no.

To say GluetrapDesc:
	if the item described is triggered:
	...
	otherwise:
		if the item described is organic:
...
Definition: a glue (called G) is too boring:
	decide no.

Definition: a glue (called G) is stealable:
	decide no.

Definition: a glue (called G) is untransformable:
	decide yes.
...
This is the spawn initial hotel glue traps rule:
	let G be the number of off-stage glue traps;
	if diaper quest is 0:
		repeat with N running from 1 to 1:
...
[This is the spawn initial jungle glue traps rule:
	...
	let N be 1;
	while G is not 0 and N < 10: [The N part is just me being defensive.]
		increase N by 1;
...
To check glue tripping:
	...
	if G is a glue:
		...
		if debugmode is 1, say "###debug: Check glue tripping: X = [X], dex = [Dex], N = [N]; GS = [GS], BaseDiff =[BaseDiff], HairDiff = [HairDiff], HeelsDiff = [HeelsDiff], TS = [TS], Flat Dex = [flat dexterity of the player][line break]";
		if X < 0:
			...
			otherwise:
				...
				otherwise:
					...
					otherwise if N > GS + BaseDiff:
						say "[one of]Bugger! Your [printed name of H] twists underfoot as you try to get to your feet. [variable custom style]No! I can't let that happen! [roman type]You almost dislocate your hip, wrenching your leg back.[or]Your [printed name of H] almost slips further into the glue.[or]Drat! I would've escaped then, if I was better at wearing high heels![at random]";
					otherwise if N > GS:
						say "[one of][variable custom style]Dammit! If I'd been on my feet instead of kneeling like a submissive [if the player is male]sissy[otherwise]slut[end if], I would've pulled free then.[roman type][or]You sob, on your knees in the glue.[or][variable custom style]I hate this stupid glue![roman type][at random]";
					otherwise:
...
A time based rule (this is the glue-stuck-mutation rule):
	if the player is glue stuck:
		[if the player is flying:
			...
			otherwise:
				say "[one of]You bob blissfully above the puddle of glue[or]You feel a bit worried about the glue puddle below you[or][first custom style]I hope I don't land in the glue when I finally descend![roman type][or]The glue puddle seems to draw your eye[or]The glue puddle beckons to you.[stopping]";
				stop;]
...
[!<RawGlueEscapingWith>+
 ...
 +]
To compute raw glue escaping (G - a glue) with (consequences - a number):
	if G is glue: [just in case it's called with "a random glue in the location of the player": which might evaluate to nothing,]
		if consequences is 1:
			...
			if d is 0:
				DelicateUp 1;
				say "[variable custom style]Ow![roman type][line break]";
		...
		if T > bond-grip: [Less than this and it's a fail]
			...
			if drama > 0:
				...
				if drama > 6:
					...
					otherwise:
						say "Nooo! You fall back into the glue!  [variable custom style]No, no, no![roman type][line break]";
		...
		if the active-colour of G is pinkish:
			...
			otherwise:
				...
				otherwise if X is 3:
					...
					IntDown 1;
				otherwise if X is 4:
					if the player is male and the size of penis > 0:
		...
		otherwise if the active-colour of G is whiteish:
			...
			otherwise if X is 2:
				if the semen taste addiction of the player < 20:
					...
					say "[one of]You suddenly realise what this glue's interesting smell reminds you of: semen![or]Mmmm![or]Mmm, yum![stopping] The aroma seems to send tingles from your nose up into your brain. [one of]You[or]Once again, you[stopping] find yourself licking your lips....";
					[I think "licking your lips" is more suggestive than the rather blunt text below. It lets the player fill in the blanks.]
					["a bit more [if the semen taste addiction of the player < 7]curious[otherwise if the semen taste addiction of the player < 14]eager[otherwise]desperate[end if] to taste some semen.[roman type][line break]";]
			otherwise if X is 3:
				let C be a random white diagram briefs;
...
To carry out PartialLatexing:
	let O be a random worn not latex clothing;
	let N be a random wrist bond;
	if O is an overdress:
		let N be a random latex overdress;
	otherwise if O is an knickers:
		let N be a random latex knickers;
	otherwise if O is an bra:
		let N be a random latex bra;
	otherwise if O is an hood:
		let N be a random latex hood;
	otherwise if O is an heels:
		let N be a random latex thigh high boots;
	otherwise if O is an stockings:
		let N be a random latex stockings;
	otherwise:
		say "The glue around your wrists turns into tough rubber, binding them together!";
		now O is nothing;
	unless O is nothing:
		say "You feel the material of your [printed name of O] saturating with the rubbery glue, and changing before your eyes into a [printed name of N]. ";
		destroy O;
	summon N cursed.
...
To compute GlueClothing (C - a clothing) with (G - a glue):
	now C is glued;
	if C is worn:
		if C is a diaper and a random number between 1 and 10 is 1:
			say "[one of]There's a crinkling sound as the glue spreads under your diaper, shrinking the plastic slightly. You feel the edges adhere to your skin, and your eyes widen as you realise what that means - when you need to change them...?[or]You feel glue seep under the elasticated edge of your [printed name of C] and tighten against your skin. Uh oh![or]There's a crinkly, rustling sound as the glue melts into the [printed name of C], bonding it to your flesh.[i]Now[/i] how will it come off?[at random]";
		otherwise:
			say "You wipe your glue-covered hands on your [printed name of C].  [one of]Uh oh... the glue spreads under the material and you feel it stick, and set.  You tug at the edges, but that only spreads more glue and sticks it down more!  Uh oh... you seem to have glued the clothing onto yourself![or]Once again the glue soaks into the [printed name of C], sticking it to you.[or]Now you've glued your [printed name of C], too.[or][printed name of C] glued.[stopping]";
	otherwise:
		[They're rubbing something in the glue to wipe it up or scrape it up: clever!] 
		say "The [C] [if C is fluid immune]scrapes away some of the glue holding you, getting seriously coated itself.[otherwise]soaks up some of the glue holding you and seems stiffer, and sticky.[end if]";
	decrease the stickiness of the player by 1;
	if the stickiness of the player < 0:
		now the stickiness of the player is 0;
		compute glue escaping.
...
Check GlueRubbing:
	...
	if the noun is clothing:
		if the stickiness of the player is 0:
			say "[one of]You scrub at the glue with the [noun] but there's no glue left![or]There's no more glue to wipe up![or]All the glue has been wiped up.[stopping]";
	...
	otherwise if the noun is a body part:
		...
		otherwise if the noun is vagina or the noun is hips:
			...
			if there is not a worn crotch covering clothing:
		...
		otherwise if the noun is breasts:
			...
			if there is not a worn breast covering clothing:
				if the latex-transformation of the player > 3, say "You sense that if you touched even just a part of your shiny new skin to another part while this glue is sticky, it will be joined together forever, stuck there!" instead;
...
To compute glued reaction of (M - a monster):
	if a random number between 1 and 3 < 3:  [+? "and M is intelligent"? ]
		...
		if M is objectifying the player:
			now M is interested;
			if M is intelligent:
				say "[one of]You hear the [M] muttering something about 'smart-glue'...[or][first custom style]'I do love the way this smart-glue only sticks to players!'[roman type][line break][first custom style]'Mmm, a lovely little piggy stuck in smart-glue!'[roman type][line break][at random]";
			otherwise:
				say "The [M] sees your hampered vulnerability and begins oiling [him of M]self up.  [second custom style]Uh oh, that can't be a good sign![roman type][line break]";
...
Check tearing off:
	...
	if the delicateness of the player > 8, say "Ow, ow, ow!  It hurts too much, the pain is bringing tears to your eyes!" instead.
...
Carry out tearing off worn clothing:
	...
	if the dexterity of the player > a random number between 1 and 30:
		...
		if a random number between 1 and 4 is 1:
			say "Finally!  You managed to carefully tear the [C] free.";
			now C is not glued;
			try taking off C;
		otherwise:
			say "You've got more of it free - keep trying!";
	otherwise if the strength of the player > a random number between 1 and 30:
		say "Ouch! That hurts, and it's tearing the [C]. Are you sure you want to continue ripping it off by brute strength? [yesnolink] ";
		if the player consents:
			if C is metal:
				say "Ouch!  That really hurt!";
				DelicateUp 1;
			otherwise:
				say "Ouch!  That really hurt!  Oh... and the glue had adhered so strongly you hear a tearing sound...";
				DelicateUp 1;
	...
	otherwise:
		say "You couldn't get a good enough grip to tear it free, that time.".
