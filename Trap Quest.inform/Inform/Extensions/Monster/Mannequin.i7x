...
To say MannequinDesc:
	if images visible is 1:
		if item described is goth mannequin:
			display the Figure of goth mannequin;
		otherwise if item described is stylish mannequin:
			display the Figure of stylish mannequin;
		otherwise if item described is topless mannequin:
			display the Figure of topless mannequin;
		otherwise if item described is flirty mannequin:
			display the Figure of flirty mannequin;
		otherwise if item described is lycra-clad mannequin:
			display the Figure of lycra-clad mannequin;
		otherwise if item described is swimmer mannequin:
			display the Figure of swimmer mannequin;
		otherwise if item described is latex-clad mannequin:
			display the Figure of latex-clad mannequin;
		otherwise if item described is lingerie-clad mannequin:
...
To say MannequinHairColour of (M - a mannequin):
	say "[if M is stylish mannequin]silvery blonde[otherwise if M is topless mannequin]strawberry blonde[otherwise if M is flirty mannequin]cherry blonde[otherwise if M is lycra-clad mannequin]lustrous black[otherwise if M is swimmer mannequin]waxy black[otherwise]platinum blonde[end if]";
...
To compute flying player taunting of (M - a mannequin):
	if a random number from 1 to 6 is 1, say "The [M] is [one of][or]still [stopping]watching you, waiting.".
...
To compute unique climax of (M - a mannequin) of (F - asshole):
	...
	otherwise:
		...
		if image cutscenes is 1, display figure of mannequin cutscene 4;
		bore M;
		let P be a random sex toy retained by M;
		if P is sex toy:
			if a random number between the difficulty of M and 5 > the size of P * 2:
				say "The [M] [if P is anal beads]pushes all [the notches of P] into your asshole before removing her hands and apparently losing interest[otherwise]removes her hands from the [printed name of P], and seems to lose interest[end if].";
			...
			otherwise:[if she doesn't leave it inside you, she saves it for next time.]
				say "The [M] pulls the [printed name of P] out of your asshole and seems to lose interest.";
		otherwise:
			say "The [M] removes her fingers from your [asshole], and seems to lose interest.";
...
To compute anal sex of (M - a mannequin):
	...
	if there is a sex toy retained by M:
		let P be a random sex toy retained by M;
		if P is anal beads:
			say "The [M] [one of]pushes the beads in and out of your [asshole] one at a time, stretching you over and over[or]pushes each bead into your [asshole] and slowly pulls them out one by one, forcing you to experience the stretching of your sphincter over and over[or]holds one bead just inside your [asshole], keeping you as stretched and full as possible[at random][run paragraph on]";
		otherwise:
			say "[one of]The [M] keeps fucking you with the [printed name of P][or]The [M] continues pumping the [printed name of P] in and out of your [asshole][or]The [M] fucks you with the [printed name of P][or]The [M] keeps pushing the [printed name of P] in and out of your hole[at random][if the player is male], [one of]angling it against your prostate[or]keeping the tip firmly in contact with your prostate[or]repeatedly prodding your 'g-spot'[or]relentlessly pounding your prostate[at random][end if][run paragraph on]";
	otherwise:
		say "[if the player is male]The [M] [one of]keeps expertly stroking your prostate[or]keeps skilfully prodding your prostate[or]firmly rubs your prostate[or]relentlessly prodding your prostate with expert movements of her fingers[at random][otherwise]The [M] keeps [one of]expertly pressing on your g-spot through your [asshole][or]pressing that same spot inside your hole, always with that perfectly maddening amount of force[or]rubbing your g-spot through your [asshole], endlessly repeating her perfect movements[at random][end if][run paragraph on]";
	...
	otherwise if the player is female:
		say " as she [if there is a worn pussy protection clothing]rubs your clit[otherwise][one of]strokes[or]teases[at random] your clit[end if], as if milking an orgasm out of you!";
	otherwise if the size of penis > 0:
		say " as she [unless there is a worn pussy protection clothing and the size of penis > 2][one of]pumps[or]strokes[or]jerks off[at random] your [ShortDesc of penis][otherwise]massages your [ShortDesc of penis][end if], [if the size of penis < 3]as if it was a clitoris![otherwise]as if milking you![end if]";
	otherwise:
		say ", as if milking an orgasm out of you!";
...
Definition: a mannequin (called M) is requiring facial penetration:
	decide no.
...
To compute (M - a goth mannequin) entering mouth:
	...
	otherwise if the lips of face < 2 + artificial enhancements fetish:
		say "The [M] puts both hands on your cheeks, pushing until your lips pucker out. She pulls out a flesh-coloured tube of lipstick and applies several coats to your puckered lips. The feeling of having your lips pushed out lingers after she releases you...";
		if image cutscenes is 1, display figure of mannequin cutscene 1;
		LipsUp 1;
		satisfy M;
...
To compute (M - a mannequin) entering mouth:
	let H be a random off-stage rubber top hat;
	let S be a random off-stage pink scrunchie;
	let F be a random off-stage fake nails;
	if H is actually summonable and artificial enhancements fetish is 1:
		say "The [M] pulls out a tiny pink top hat and fixes it onto your head!  You immediately feel a bit more bimbo-like...";
		summon H cursed;
		satisfy M;
...
This is the mannequin donates clothing rule:
	...
	if S is actually summonable and the number of worn shoes is 0:
		summon S cursed;
		say "The [current-monster] produces a [printed name of S] and pushes them onto your feet!  Uh-oh!  ";
		satisfy current-monster;
		rule succeeds;
	let B be a random off-stage flimsy bra;
	if B is actually summonable and the number of worn dress is 0:
		summon B cursed;
		say "The [current-monster] produces a [printed name of B] and pulls it over your breasts!  Uh-oh!  ";
		satisfy current-monster;
		rule succeeds;
	let F be a random off-stage floral purple sissy dress;
	if F is actually summonable:
		summon F cursed;
		say "The [current-monster] produces a [F] and pulls it over you!  Uh-oh!  ";
		satisfy current-monster;
		rule succeeds;
	...
	if M is actually summonable:
		summon M cursed;
		say "The [current-monster] produces a [M] and pulls it up your legs onto your waist!  Uh-oh!  ";
		satisfy current-monster;
		rule succeeds.
...
This is the mannequin applies make-up in diaper quest rule:
	...
	if the make-up of face < 3:
		say "The [M] pulls out a make up kit, and starts wildly applying it to your face!  You soon look like a dolled up [if the bimbo of the player < 10]whore[otherwise]clown[end if] who clearly doesn't know how to make herself look respectful!";
...
To compute unique death of (M - a mannequin):
	...
	if a random number between 1 and 5 > 3 and J is make up kit:
		say "You notice that her [J] appears to have remained intact.";
