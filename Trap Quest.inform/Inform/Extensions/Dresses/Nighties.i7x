...
A nightie is a kind of overdress.  A nightie is usually sheer.  A nightie is usually short.  A nightie is usually silk.  A nightie is usually unique.  Understand "nightie" as nightie. A nightie is usually not-top-displacable.  A nightie is usually delicateness-influencing.

Definition: a nightie (called C) is sissifying:
	decide yes.

To decide which number is the fatigue-influence of (C - a nightie):
	let F be 1;
	decrease F by the magic-modifier of C;
	decide on F.

To say ShortDesc of (C - a nightie):
	say "nightie".
	
To compute periodic effect of (O - a nightie):
	let D be a random eligible diaper;
	if D is actually summonable and diaper lover >= 1:
		let R be 240;
		if unlucky is 1, now R is 150;
		decrease R by the urine-soak of O * 10;
		if O is cursed, decrease R by 20;
		if O is bed wetting, decrease R by 25;
		if a random number between 1 and R <= 2:
			say "[bold type]Your [printed name of O] [bold type]shimmers and a diaper appears around your loins![roman type][line break]";
			set up D;
			summon D cursed.

Report wearing a nightie:
	if busy is 0:
...
Report examining a nightie:
	say "Somehow you can tell that you'll be really quick at recovering your energy once on your knees, whilst you wear this.".

To decide which number is the initial outrage of (C - a nightie):
	decide on 7.

To decide which object is the potential-upgrade-target of (C - a nightie):
	if the number of worn diapers is 0, decide on a random off-stage negligee;
	decide on nothing.

To decide which number is the initial cringe of (C - a nightie):
	decide on 4.

Part 1 - Organza Nightie

An organza nightie is a kind of nightie.  There is 1 organza nightie.
...
The printed name of nightgown is usually "[TQlink of item described][clothing-title-before]nightgown[clothing-title-after][TQxlink of item described][verb-desc of item described]".  The text-shortcut of nightgown is "nig".

To decide what number is the price of (C - a nightgown):
	decide on 3.

Figure of nightgown is the file "nightie2.png".

To decide which figure-name is clothing-image of (C - a nightgown):
	decide on figure of nightgown.

To say ClothingDesc of (C - a nightgown):
...
To decide which number is the fatigue-influence of (C - a nightgown):
	decide on 0.
	
To compute periodic effect of (O - a nightgown):
	let D be a random eligible diaper;
	if D is actually summonable and diaper lover >= 1:
		let R be 240;
		if unlucky is 1, now R is 150;
...
Definition: a pink nightie (called C) is sissifying:
	decide no.
...
To decide which number is the initial outrage of (C - a pink nightie):
	decide on 5.

To decide which object is the potential-upgrade-target of (C - a pink nightie):
	decide on a random off-stage unique nightie.
